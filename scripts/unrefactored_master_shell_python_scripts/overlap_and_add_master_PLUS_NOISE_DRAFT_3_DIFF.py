...
class Overlap_Add():
    ...
    def empty(self, input):
        ...
        for (i, x) in enumerate(frames):
            ...
            noise_frame = noise_frame * self.create_window(2*this_peak, 2*len(new_frame))
            ...
            if i == 0:#initialises with first frame
                ...
                if hypothesis > timestamp + 0.0025:
                    ...
                    if join_position + len(new_frame) <= len(so_far):
                        ...
                        add_noise_frame = np.append(np.zeros(2*join_position), noise_frame)
                        ...
                        add_frame = np.append(np.zeros(join_position), new_frame)
                        ext_noise = np.append(self.all_noise, np.zeros(2*len(new_frame)-2*avg_length))
                        add_noise_frame = np.append(np.zeros(2*join_position), noise_frame)

                    if len(add_noise_frame) < 2*len(add_frame):
                        add_noise_frame = np.append(np.zeros(1), add_noise_frame)
                    if len(add_noise_frame) > 2*len(add_frame):
                        add_noise_frame = add_noise_frame[1:]
                    if len(ext_noise) < 2*len(ext_prev_mast):
                        ext_noise = np.append(ext_noise, np.zeros(1))
                    if len(ext_noise) > 2*len(ext_prev_mast):
                        ext_noise = ext_noise[:-1]

                    self.master = ext_prev_mast + add_frame
                    #print "master"
                    #print len(ext_prev_mast), "    ", len(add_frame)
                    #print len(ext_noise), "    ", len(add_noise_frame)
                    ...
                    while hypothesis < timestamp - 0.0025:
                        ...
                        if join_position + len(prev_frame) <= len(so_far):
                            ...
                            ext_noise = self.all_noise
                            add_noise_frame = np.append(np.zeros(2*join_position), prev_noise)
                            #print "EXT_PREV_MAST =    ", len(ext_prev_mast)
                            #print "ADD FRAME =     ", len(add_frame)
                        else:
                            #print "Extension ==="
                            ...
                            add_frame = np.append(np.zeros(join_position), prev_frame)
                            ext_noise = np.append(self.all_noise, np.zeros(2*len(prev_frame)-2*avg_length))
                            add_noise_frame = np.append(np.zeros(2*join_position), prev_noise)
        ...
        self.master = scipy.signal.resample(self.master, 2*len(self.master))
        ...
        #noise_hi_pass = scipy.signal.resample(noise_hi_pass, 2*len(noise_hi_pass))

        self.master = master_lo_pass + (0.25*noise_hi_pass) #(self.all_noise/5)
    ...
    def reconstruct_to_wav(self, arr):
        ...
        scipy.io.wavfile.write(str(args.input)+'_abs_noise_16000_DEBUG_2s_lpl_BUTTER_quarter_param2.wav', 16000, self.normed)
