...
class Overlap_Add():
    ...
    def reconstruct_to_wav(self, arr):
        ...
        if args.preemphasis == True:
            scipy.io.wavfile.write(str(args.input)+'_8khz_no_noise_preemph.wav', int(self.samplerate), self.normed)
        else:
            scipy.io.wavfile.write(str(args.input)+'_8khz_no_noise.wav', int(self.samplerate), self.normed)
