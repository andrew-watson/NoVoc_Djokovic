...
class Proc():
    ...
    def get_time_ranges(self, tracker):
        ...
        #Corresponding linguistic features are extracted at 5ms intervals, so there must be a 1:1 mapping between linguistic and acoustic frames#
        for j in numpy.arange(0.000, float(args.duration), 0.005):
            ...
            #ORIGINAL PITCHMARK SAMPLE INDEX#
            self.current = self.tracker[i][0]

            #Extract time boundaries (ms) of current frame#
            #Reapeated condition if a different fixed frame shift (instead of REAPER's 10ms) is to be used#
            ...
            #Find indeces of frame beginning, current peak, and frame end##
    ...
    def naming_convention(self, name):
        """Fancy naming convention formatting"""
